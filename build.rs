...
fn main() {
    ...
    // instead of failing. `cargo test` then reports the remaining suite green.
...
/// directory. Same resolution order the runtime tests use: prefer
/// `DUCKLINK_CORPUS_DIR`, fall back to the monorepo default. An empty (0-byte)
/// file counts as absent, since a common monorepo state is a stub-committed
/// artifact rebuilt on demand — running against a 0-byte blob would fail with
/// a wasmtime error, not a clean skip.
mod corpus_probe {
