    ...
    impl VScalar for AbaValidateNative {
        ...
        fn invoke(
            ...
            let mut out = output.flat_vector();
            {
                ...
                for i in 0..len {
                    // NULL propagation (the semantic contract: NULL in -> NULL out).
                    // Never read a NULL row's string_t (it may be garbage); write a
                    // placeholder, then set_null below.
                    out_slice[i] = if in_vec.row_is_null(i as u64) {
                        false
                    } else {
                        let mut t = strs[i];
                        aba_checksum_valid(&DuckString::new(&mut t).as_str())
                    };
                }
            }
            for i in 0..len {
