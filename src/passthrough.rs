...
/// Canonical conformance-suite digest for `name`, byte-identical to build C's
/// `resolver::compute_suite_digest` / `tooling/conformance.py`: a structured
/// digest over `$home/suites/<name>.sql` + `<name>.expected`. `None` if absent
/// -> the gate's suite-digest sub-check is skipped (passed + contract still
/// enforced). The shim and the host MUST agree on this scheme or certified
/// records would be rejected as stale.
fn canonical_suite_digest(home: &std::path::Path, name: &str) -> Option<String> {
    let dir = home.join("suites");
    let sql = std::fs::read_to_string(dir.join(format!("{name}.sql"))).ok()?;
    let expected = std::fs::read_to_string(dir.join(format!("{name}.expected"))).ok()?;
